...
module.exports = (io, db) => {
  io.on("connection", (socket) => {
    ...
    OnlineUsersStore[socket_user.username] = {socketId : socket.id, userId: socket_user._id}
    ...
    socket.on("chat message", (msg, username) => {
        ...
        messages_collection.findOne({user_id: socket_user._id}, (err, doc)=>{
          ...
          messages_collection.updateOne({username: socket_user.username}, {$set:{message_threads:obj}}, (err)=>{
            messages_collection.findOne({username: username}, (err, doc)=>{
              var update_obj = updateArrayInMessageThreads(doc, socket_user.username, msg, false)
              
              messages_collection.updateOne({username:username}, {$set:{message_threads: update_obj}})

            })
          });
        })
        socket.to(
        OnlineUsersStore[username]["socketId"]
        ).emit("receive message",{from:socket_user.username, message: {text:msg, self:false}});
